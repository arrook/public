<template>
  <a-card size="small" title="开机欢迎语">
    <a-form-item>
      <a-switch v-model:checked="model.enabled" disabled />
      <span style="margin-left: 8px">启用开机播报</span>
    </a-form-item>

    <template v-if="model.enabled">
      <a-form-item label="欢迎语文本">
        <a-textarea
          v-model:value="model.voice_text"
          placeholder="系统启动完成后播放的语音内容"
          :rows="3"
          disabled
        />
      </a-form-item>

      <a-form-item label="语音编号">
        <a-select v-model:value="model.voice_music" disabled>
          <a-select-option :value="1">语音1</a-select-option>
          <a-select-option :value="2">语音2</a-select-option>
          <a-select-option :value="3">语音3</a-select-option>
        </a-select>
      </a-form-item>
    </template>
  </a-card>
</template>

<script setup lang="ts">
import { computed } from 'vue'
import type { StartTextConfig } from '@/types/config'

const props = defineProps<{
  modelValue: StartTextConfig
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', v: StartTextConfig): void
}>()

const model = computed({
  get: () => props.modelValue,
  set: v => emit('update:modelValue', v)
})
</script>